...
urlservice = "/admin/registration"#service for the authorized redirect
...
@rauth.route('/login')
def login():
    return google.authorize(callback=url_for('rauth.authorized', _external=True))

@rauth.route('/login/authorized')
def authorized():
...
app.register_blueprint(rauth, url_prefix='/rauth')
